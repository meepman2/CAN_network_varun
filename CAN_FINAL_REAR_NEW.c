...
ADC_Set_Input_Channel(_ADC_CHANNEL_10 | _ADC_CHANNEL_11 | _ADC_CHANNEL_12 | _ADC_CHANNEL_13 | _ADC_CHANNEL_14 | _ADC_CHANNEL_15 );
  ...
  CANSetFilterScale32(1, _CAN_FILTER_ENABLED & _CAN_FILTER_ID_MASK_MODE & _CAN_FILTER_STD_MSG, 1534, -1);
  ...
  while (1) {                                                                  // endless loop
    ...
    if ((Rx_ID==1534) &&Msg_Rcvd)                                                 // if message received
    {
      Delay_us(250);
      
      CAN1Write(1535, send_data1, 8, Can_Send_Flags);     // send message set1
      CAN1Write(1536, send_data2, 8, Can_Send_Flags);
      CAN1Write(1537, send_data3, 2, Can_Send_Flags);
      //Delay_us(300);


      Write(1538, send_data4, 8, Can_Send_Flags);     // send message set2
      Write(1539, send_data5, 8, Can_Send_Flags);
      Write(1540, send_data6, 2, Can_Send_Flags);
      //Delay_us(300);


      Write(1541, send_data7, 8, Can_Send_Flags);     // send message set3
      Write(1542, send_data8, 8, Can_Send_Flags);
      Write(1543, send_data9, 2, Can_Send_Flags);
      //Delay_us(300);


      Write(1544, send_data10, 8, Can_Send_Flags);     // send message set4
      Write(1545, send_data11, 8, Can_Send_Flags);
      Write(1546, send_data12, 2, Can_Send_Flags);
      //Delay_us(300);
