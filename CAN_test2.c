...
void main() {
  ...
  while (1) {                                                                  // endless loop
    ...
    if ((Rx_ID==1525) &&Msg_Rcvd)                                                 // if message received
    {
      ...
      CAN1Write(1526, send_data, 8, Can_Send_Flags);
      CAN1Write(1527, send_data, 8, Can_Send_Flags);
      CAN1Write(1528, send_data, 8, Can_Send_Flags);
      GPIOB_ODR.b0 = 1;
       }
    ...
    if( j==1)
    {
      CAN1Write(1534, send_data, 6, Can_Send_Flags);
      CAN1Write(1531, send_data, 8, Can_Send_Flags);
      CAN1Write(1532, send_data, 4, Can_Send_Flags);
      GPIOB_ODR.b0 = 0;
      j = 0 ;
  }
  else
